...
def make_local_matrix(k, C, mu, u_border, t, t_border, elements, vertices):
...
# Вычисление локального вектора правой част
def make_local_vector(k, mu, t, t_border, elements, vertices):
    k0, k1, k2 = elements[k] # получаем номера вершин
...
for k in range(2 * X * Y):
    f_k = make_local_vector(k, mu, t, t_border, elements, vertices)
...
for k in range(2 * X * Y):
    G_k = make_local_matrix(k, C, mu, u_border, t, t_border, elements, vertices)
    g = gl_indexes(k, elements)
    for i in range(6):
